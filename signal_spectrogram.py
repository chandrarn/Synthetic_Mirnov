...
def multiple_spect(params=[{'m':12,'n':10,'f':50e3,'dt':0},{'m':2,'n':1,'f':7e3,'dt':0}],
                   filament=[True,True],save_Ext='',phi_sensor=[[180],[0]],
                     sensor_file='MAGX_Coordinates_CFS.json',sensor_set=['MRNV','BP'],
                     file_geqdsk='geqdsk',doVoltage=False,doSave='',f_lim=[180,0]):
    ...
    for ind,param in enumerate(params):
        ...
        if ind == 0: spects=out_spect
        else:spects +=out_spect
...
def gen_single_spect(params,filament,save_Ext,phi_sensor=[180],
    ...
    time = time[0]
    signals = signals[0]
    time, freq, out_spect = rolling_spectrogram(time, signals,pad=250)
...
def plot_spectrogram(time,freq,out_spect,doSave,sensor_set,params,filament,save_Ext,f_lim):
    ...
    plt.show()
    fName ='floops_%s_%s_m-n_%d-%d_f_%d%s.pdf'%\
