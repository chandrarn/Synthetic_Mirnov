...
NUM_CONFOUND_LINES = (0,5)
CONFOUND_LINE_WIDTH = (1,3)
...
class SyntheticSegmentationDataset(Dataset):
    def __init__(self, num_samples, height, width, transforms=None):
        """
        Args:
            num_samples (int): Number of synthetic image-mask pairs to generate.
            height (int): Desired height of the images.
            width (int): Desired width of the images.
            transforms (callable, optional): Optional transform to be applied
                on a sample. Expected to handle both image and mask.
        """
        ...
        # Generate all samples upfront (for a small number of samples)
        # For very large datasets, you might generate on-the-fly in __getitem__
        self.data = []
    ...
    def __getitem__(self, idx):
        ...
        if self.transforms:
            # Apply transforms. Note: transforms should be careful with masks
            # E.g., convert to tensor, normalize image, keep mask as is (or to LongTensor)
            # A common approach is to convert both to tensors first.
            image = transforms.ToTensor()(image) # Converts PIL Image to FloatTensor [0,1], C x H x W
            mask = transforms.ToTensor()(mask)   # Converts PIL Image (L) to FloatTensor [0,1], C x H x W
            
            # For segmentation, masks are typically LongTensor with class IDs
            # Here, we have binary masks (0 or 1), so convert to Long
            mask = (mask * 255).to(torch.long) # Scale back to 0 or 255, then to Long
...
def visualize_sample(image_tensor, mask_tensor, title=""):
    ...
    """
    plt.close(title)
    fig, axes = plt.subplots(1, 2, figsize=(10, 5),num=title)
    fig.suptitle(title)
