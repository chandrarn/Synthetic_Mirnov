...
# -*- coding: utf-8 -*-
"""
Created on Fri Jun 20 13:36:22 2025
...
IMAGE_HEIGHT = 128
IMAGE_WIDTH = 128
NUM_SAMPLES = 15 # Increased samples for better Faster R-CNN training
...
# --- Training Loop ---
NUM_EPOCHS = 2 # Adjusted epochs for Faster R-CNN
...
with torch.no_grad():
    ...
    fig, axes = plt.subplots(1, 1, figsize=(8, 8),num='Sample Image Validation')
    axes.imshow(sample_image_np)
    axes.set_title('Validation Sample: Input Image with True and Predicted BBoxes')
    axes.axis('off')
    ...
    # Draw all predicted bounding boxes (filter by score if desired)
    SCORE_THRESHOLD = 0.5 # Only show predictions with confidence above this threshold
...
#     axes.set_title(f'Inference: {os.path.basename(img_paths[0])} with Predicted BBoxes')
#     axes.axis('off')

#     # Draw all predicted bounding boxes (filter by score)
#     SCORE_THRESHOLD = 0.5 # Confidence threshold for displaying bounding boxes
