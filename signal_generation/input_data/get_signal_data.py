...
from header_signal_analysis import (
    histfile,
    np,
    json,
    geqdsk,
...
def get_signal_data(
    ...
    hist_file = histfile(
        "../data_output/floops_%s_%s_m-n_%d-%d_f_%d%s.hist"
        % (
            "filament" if filament else "surface",
            sensor_set,
            params["m"],
            params["n"],
            params["f"] * 1e-3,
    ...
    print(
        "Loaded: ../data_output/floops_%s_%s_m-n_%d-%d_f_%d%s.hist"
        % (
            "filament" if filament else "surface",
            sensor_set,
            params["m"],
    ...
    sensor_dict = __select_sensors(
        sensor_set, sensor_params, phi_sensor, file_geqdsk, params
    )
    # return sensor_dict, sensor_params, hist_file
    # build datasets
    X, Y, Z = __gen_surface_data(
        sensor_dict, hist_file, doVoltage, params, sensor_set, sensor_params
    )
    return X, Y, Z, sensor_dict


def __gen_surface_data(
    sensor_dict, hist_file, doVoltage, params, sensor_set, sensor_params
):
    # Build 2D data
    dt = params["dt"]
        ...
        Z_tmp = np.array(
            [
                (
                    ...
                    if doVoltage
                    else hist_file[s_["Sensor"]][:] * 1e4
                )
                for s_ in s
            ]
        ).squeeze()
        # length check
        if Z_tmp.shape[1] > X[0].shape[0]:
            Z_tmp = Z_tmp[:, :-1]
...
def __select_sensors(
    sensor_set, sensor_params, phi_sensor, file_geqdsk, params, shotno=None, tLim=None
):
    ...
    if sensor_set == "MIRNOV":
        ...
        for s in subset:
            ...
            if s[0] == "V":
                sensor_dict[0].append(
                    {
                        "Sensor": "%s_%s_%s"
                        % (sensor_set, "TOR_SET_%03d" % phi_sensor[0], s),
                        "y_vals": theta,
                        "y_label": r"Mirnov-V $\theta$ [deg]",
                    }
                )
            if s[0] == "H":
                sensor_dict[1].append(
                    {
                        "Sensor": "%s_%s_%s"
                        % (sensor_set, "TOR_SET_%03d" % phi_sensor[0], s),
                        "y_vals": PHI,
                        "y_label": r"Mirnov-H $\phi$ [deg]",
                    }
                )
        ...
        for s in subset:
            ...
            # separate sets
            if not (phi_sensor[0] - 20 <= PHI <= phi_sensor[0] + 20):
                continue
                ...
                sensor_dict[0].append(
                    {
                        "Sensor": "%s" % (s),
                        "y_vals": theta,
                        "y_label": r"%s$_{\phi=%d}\,\hat{\theta}$ [deg]"
                        % (sensor_set, PHI),
                    }
                )
                ...
                sensor_dict[1].append(
                    {
                        "Sensor": "%s" % (s),
                        "y_vals": PHI,
                        "y_label": r"%s$_{\theta=%d}\,\hat{\phi}$ [deg]"
                        % (sensor_set, theta),
                    }
                )
    ...
    ###########################################################################
        ...
        for i in np.arange(1, 4):
            for ind, set_ in enumerate(["AB", "GH"]):
                ...
                sensor_dict[ind].append(
                    {
                        "Sensor": name,
                        "y_vals": sensor_params[name],
    ...
    ###########################################################################
            ...
            sensor_dict[-1].append(
                {
                    ...
                    "y_label": r"%s$_{\phi=%d}\,\hat{\theta}$ [deg]"
                    % (sensor_set, PHI[ind]),
                    "Signal": sensor["SIGNAL"][ind, tInd]
                    * (1 if 0 <= theta[ind] <= 180 else -1),
                    "Time": time[tInd],
                }
            )
        ...
        for ind_, s in enumerate(sensor_sets):
            if ind_ == ind:
                continue  # No need to double count sensor from poloidal array
            ...
            sensor_dict[-1].append(
                {
                    "Sensor": "%s" % (s["NAMES"][tor_ind]),
                    "y_vals": s["PHI"][0],
                    "y_label": r"%s$_{\theta=%d}\,\hat{\phi}$ [deg]" % (sensor_set, 0),
                    "Signal": s["SIGNAL"][tor_ind, tInd],
                    "Time": time[tInd],
                }
            )
    ###########################################################################
        ...
        try:
            ...
            print("Error loading EFIT tree: ", e)
            eq = CModEFITTree(1160930034)
        ...
        for i in np.arange(1, 4):
            sensor_dict[-2].append(
                {
                    "Sensor": "%s" % (sensor.ab_names[i]),
                    "y_vals": sensor.ab_phi[i] + 360,
                    "y_label": r"%s$_{\theta=%d}\,\hat{\phi}$ [deg]"
                    % (sensor_set, theta),
                    "Signal": np.cumsum(sensor.ab_data[i - 1])[tInd] * dt,
                    "Time": time[tInd],
                }
            )
            ...
            sensor_dict[-1].append(
                {
                    "Sensor": "%s" % (sensor.gh_names[i]),
                    "y_vals": sensor.gh_phi[i] + 360,
                    "y_label": r"%s$_{\theta=%d}\,\hat{\phi}$ [deg]"
                    % (sensor_set, theta),
                    "Signal": np.cumsum(sensor.gh_data[i - 1])[tInd] * dt,
                    "Time": time[tInd],
                }
            )
        ...
        raise SyntaxWarning("Diagnostic Not Implimented")
    return sensor_dict
...
def field_to_current(B, dt, w_mode, sensor_params, sensor_set, sensor_name):
    ...
    if sensor_set != "MIRNOV":
        ...
        wc = (
            23.1875 * 1e3 * 2 * np.pi
        )  # fitted critical frequency from emperical sensor tests
    else:
        NA = sensor_params[sensor_set][sensor_name[7:18]][sensor_name[19:]]["NA"]["NA"][
            0
        ]
        wc = 2e6 * 2 * np.pi

    # Signal damping factor, in SI units
    def factor(NA, wc, w_mode):
        return -1 * NA / (1 + (w_mode * 2 * np.pi) ** 2 / wc**2)
