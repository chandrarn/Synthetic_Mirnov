...
def main():
    ...
    plt.close("test")
    fig, ax = plt.subplots(
        1, 2, num="test", tight_layout=True, subplot_kw={"projection": "3d"}
    )
    ...
    for ind, line in enumerate(coords):
        ...
        ax[0].plot(
            *line.T,
            c=color,
            alpha=1 if ind == (len(coords) - 1) else other_lines,
            lw=10 if ind == 0 else 1,
        )
        ...
        ax[1].plot(
            *coords_old[ind],
            c=color_old,
            alpha=1 if ind == (len(coords_old) - 1) else other_lines,
            lw=10 if ind == 0 else 1,
        )
...
def gen_filament_coords(params):
    ...
    n_local = ratio.denominator
    return np.linspace(
        0, 2 * np.pi / m_local * n_local, m_pts, endpoint=True
    ), np.linspace(0, m_local * 2 * np.pi, n_pts, endpoint=True)


####################################
def calc_coords_improved(
