...
cos_current = (
    timebase_current / mode_growth * np.cos(mode_freq * 2.0 * np.pi * timebase_current)
)
cos_voltage = np.diff(cos_current) / np.diff(timebase_current)
sin_current = (
    timebase_current / mode_growth * np.sin(mode_freq * 2.0 * np.pi * timebase_current)
)
...
for i in range(nsteps + 2):
    ...
    if i > 0:
        volt_full[i, 1:] = mode_driver[0, :] * np.interp(
            volt_full[i, 0], timebase_voltage, cos_voltage
        ) + mode_driver[1, :] * np.interp(
