...
def Synthetic_Mirnov_Surface(
    ...
    _sensors = gen_Sensors_Updated(select_sensor=sensor_set, cmod_shot=cmod_shot)

    # Generate 2D b-norm sin/cos
    if C1_file:
        convert_to_Bnorm(C1_file, params["n"], params["n_pts"])
    elif FAR3D_br_file:
...
########################################################
def __run_td(
    mode_driver_flux, sensor_mode, tw_mesh, sensor_obj, params, sensor_set, save_Ext
):
    dt = params["dt"]
    ...
    if type(params["f"]) is float:
        mode_freq = params["f"] * 2 * np.pi * timebase_current  # always just return f
    # assume frequency is a function taken [0,1] as the argument
    else:
        mode_freq = params["f"](timebase_current)

    if type(params["I"]) is int:
        mode_amp = params["I"] * np.ones((nsteps + 1,))
    else:
        mode_amp = params["I"](timebase_current)
    ...
    for i in range(nsteps + 2):
        ...
        if i > 0:  # Leave 0-th step voltage, current, as zero
            volt_full[i, 1:] = mode_driver_flux[0, :] * np.interp(
                volt_full[i, 0], timebase_voltage, cos_voltage
            ) + mode_driver_flux[1, :] * np.interp(
                volt_full[i, 0], timebase_voltage, sin_voltage
            )

            sensor_signals[i, 1:] = sensor_mode[0, :] * np.interp(
                volt_full[i, 0], timebase_current, cos_current
            ) + sensor_mode[1, :] * np.interp(
                volt_full[i, 0], timebase_current, sin_current
            )
...
def run_frequency_scan(
    ...
    for freq in freqs if np.ndim(freqs) > 0 else [freqs]:
        ...
        probe_signals = np.dot(result, Msensor) + np.dot(
            mode_drive_currents, Msensor_plasma
        )
    ...
    if doSave_Bode:
        ...
        fName = (
            "Frequency_Scan_for_%s_on_%s_using_%s_from_%2.2e-%2.2eHz_n=%d_rho=%1.1f_lambda=%1.2f%s_Surface_Current.npz"
            % (
                file_geqdsk,
                mesh_file,
                sensor_set,
...
def get_mesh_and_driver(mesh_file, oft_env, sensor_set):
    ...
    tw_mesh.setup_model(
        mesh_file="input_data/" + mesh_file, xml_filename="input_data/oft_in.xml"
    )
    tw_mesh.setup_io()
    ...
    # Mc = (
    #     tw_mesh.compute_Mcoil()
    # )  # Inductance to the coils not necessary for surface current case
    ...
    Msensor_plasma, _, _ = tw_mode.compute_Msensor(
        "input_data/floops_%s.loc" % sensor_set
    )
...
#########################################################
def __gen_b_norm_mesh(
    C1_file, n_theta, n_phi, oft_env, sensor_set, doSave, save_Ext, params, doPlot
):
    ...
    r_grid, bnorm, nfp = build_torus_bnorm_grid(
        fname, n_theta, n_phi, resample_type="theta", use_spline=False
    )
...
###########################################################
def __b_norm_surf_plot(
    r_grid, bnorm, nfp, ntheta, nphi, doSave="", save_Ext="", params={}
):
    # 2D Plot of B-norm and J
    theta = np.linspace(-np.pi, np.pi, ntheta, endpoint=False) / np.pi
    # phi = (
    #     np.linspace(0, 2 * np.pi, (nphi - 1) if nfp > 1 else nphi, endpoint=False)
    #     / np.pi
    # )
    ...
    # redo_inds = np.concatenate(
    #     (np.arange(int(len(theta) / 2), len(theta)), np.arange(0, int(len(theta) / 2)))
    # )
    ...
    for i in range(2):
        ax[0, i].set_xlabel(r"$\phi$ [$\pi$-rad]")
    ...
    if doSave:
        fig.savefig(
            doSave
            + "Surface_Current_2D_%s_m-n_%d-%d_%s.pdf"
...
def __do_plot_B_J(
    ...
    celltypes = np.array(
        [pyvista.CellType.TRIANGLE for _ in range(lc.shape[0])], dtype=np.int8
    )
    cells = np.insert(
        lc,
        [
    ...
    actor = p.add_mesh(
        ...
        show_edges=True,
        scalars=bn_c,
        scalar_bar_args={"title": r"$\sigma(\theta,\phi)\, [A/m]$"},
    )
    ...
    p.add_mesh(grid, color="white", opacity=0.3, show_edges=True)
    ...
    if doSave:
        p.save_graphic(
            doSave
            + "Surface_J_s_%s_m-n_%d-%d_f_%d%s.pdf"
            % (
                sensor_set,
                params["m"] if type(params["m"]) is int else params["m"][0],
                params["n"],
                params["f"] * 1e-3,
                save_Ext,
            )
    ...
    if doSave:
        p.save_graphic(
            doSave
            + "Surface_J_s_%s_m-n_%d-%d_f_%d%s.svg"
            % (
                sensor_set,
                params["m"] if type(params["m"]) is int else params["m"][0],
                params["n"],
...
if __name__ == "__main__":
    ...
    # mesh_file='SPARC_Sept2023_noPR.h5';
    doSave = "../output_plots/"
    save_ext = "_FAR3D_NonLinear_Low_Mesh_Resolution"
    sensor_set = "BP"
    xml_filename = "oft_in.xml"
    ...
    FAR3D_br_file = "br_7029"
    FAR3D_bth_file = "bth_7029"
    ...
    Synthetic_Mirnov_Surface(
        ...
        doPlot=doPlot,
        C1_file=C1_file,
        FAR3D_br_file=FAR3D_br_file,
        FAR3D_bth_file=FAR3D_bth_file,
        plotOnly=plotOnly,
        eta=eta,
