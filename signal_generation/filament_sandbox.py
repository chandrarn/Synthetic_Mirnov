...
path.append("/home/rianc/Documents/TARS/")
...
def gen_contours(file_geqdsk, doDebug=True):
    # Load eqdsk
    print(getcwd())
    with open("input_data/" + file_geqdsk, "r") as f:
        eqdsk = geqdsk.read(f)
    ...
    r_theta = make_smoothing_spline(
        theta_r[arg_sort_theta_r], r_norm[arg_sort_theta_r], lam=None
    )
    ...
    Br, Bt, Bz = eqilib_field.get_field_at_point(
        r_theta(theta_r) * np.cos(theta_r) + rmagx,
        r_theta(theta_r) * np.sin(theta_r) + zmagx,
    )
    Bp = np.sqrt(Br**2 + Bz**2)

    # Need a function to extract Bp, Bt as functions of theta, to later find d_theta

    Bp_theta = make_smoothing_spline(
        theta_r[arg_sort_theta_r], Bp[arg_sort_theta_r], lam=None
    )
    Bt_theta = make_smoothing_spline(
        theta_r[arg_sort_theta_r], Bt[arg_sort_theta_r], lam=None
    )
...
def wind_in_phi(
    ...
    for i, known_theta_start in enumerate(known_theta[:-1]):
        ...
        theta_indices = np.where(
            (theta >= known_theta_start) & (theta <= known_theta_end)
        )

        actual_theta_start = theta[theta_indices[0]]
        actual_theta_end = theta[theta_indices[-1]]
        correction_factor = (known_theta_end - known_theta_start) / (
            actual_theta_end - actual_theta_start
        )
        theta[theta_indices] = (
            known_theta_start
            + (theta[theta_indices] - actual_theta_start) * correction_factor
        )

    for phi in phi_start:
        phi_ = (
            phi + phi_advance
        )  # All phi points from a given starting point, of size m_pts
...
def debug_plots(coords, theta, phi, m, n, d_th):
    ...
    ax1.plot(
        np.array([phi[0], phi[-1]]) / (2 * np.pi),
        np.array([theta[0], theta[-1]]) / (2 * np.pi),
        "--k",
    )
...
def wind_in_theta(file_geqdsk, m, n):
    ...
    filament_points, filament_etas = eq_filament._trace(
        method=TraceType.AVERAGE, num_filament_points=300
    )
...
def calc_filament_coords_field_lines(params, file_geqdsk, doDebug=False):
    ...
    for ind_m, m_ in enumerate(m if type(m) is list else [m]):
        ...
        if doDebug:
            debug_filament_currents(coords_, phi_start, n[ind_m])
        # coords_[:,0]= coords[:,1]
