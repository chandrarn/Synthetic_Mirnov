...
def gen_mode_params(
    ...
    for _ in range(training_shots):
        ...
        I_mod = lambda t: 0.1 * 4 * (5 + 7 * t**4)

        f_out_1, I_out_1, f_out_plot_1 = gen_coupled_freq(
            time, periods, dead_fraction, f_mod, I_mod, random_seed=42
        )

        periods = 2
        dead_fraction = 0.2
        f_mod = lambda t: 35e3 + 5e3 * t
        I_mod = lambda t: 0.05 * (6 + 3 * np.sin(periods * 2 * np.pi * t))

        f_out_2, I_out_2, f_out_plot_2 = gen_coupled_freq(
            time, periods, dead_fraction, f_mod, I_mod, random_seed=42
        )
        ...
        def f_mod(t):
            return 200e3 - 5e3 * t**2

        f_out_3, I_out_3, f_out_plot_3 = gen_coupled_freq(
            time, periods, dead_fraction, f_mod, I_mod, random_seed=42
        )
        ...
        params["f"] = [
            ...
            f_out_2,
            f_out_3,
        ]  # List of frequencies for each m/n pair
        params["I"] = [
            I_out_1,
            I_out_2,
            I_out_3,
        ]  # List of amplitudes for each m/n pair
...
######################################################################################################
def gen_mode_params_for_training(
    training_shots=1,
    params={
        "T": 1e-3,
        "dt": 1e-7,
    ...
    for shot_ind, shot in enumerate(range(training_shots)):
        ...
        for n in params["n"]:
            ...
            f_local = gen_frequency_evolutions(
                n, params["m"][params["n"].index(n)], doPerturbation=doPerturbation
            )
            # Generate amplitude evolution for each mode
            I_local = gen_amplitude_evolution(
                np.mean(f_local(np.array([0, 1]))), doPerturbation
            )  # Use the average frequency for amplitude scaling

            # Generate the time-dependent frequency and amplitude
            dead_fraction = np.random.uniform(0.1, 0.5)
            periods = np.random.randint(
                1, int(1 / dead_fraction)
            )  # Randomly choose number of periods
            f_local, I_local, f_plot_local = gen_coupled_freq(
                time, periods, dead_fraction, f_local, I_local
            )
            ...
            params["f_Hz"].append(
                f_plot_local
            )  # f in Hz for plotting and training comparison
...
######################################################################################################
def gen_frequency_evolutions(
    n, m, v_phi_0=[20, 40], v_a=[200, 400], doPerturbation=True
):
    """
    ...
    # 1. Base Frequency
    f_0 = (
        n * rng.uniform(v_phi_0[0], v_phi_0[1]) * 1e3
    )  # Approximate Doppler shifted frequency, in Hz
    if n >= 6:  # AE
    ...
    if evolution_type == "linear":
        slope = (
            rng.uniform(-0.1, 0.1) * f_0
        )  # Linear slope (fraction of base frequency)

        def f_evol(time):
            return slope * time + f_0
    else:  # polynomial
        a = (
            rng.uniform(-0.005, 0.005) * f_0
        )  # Quadratic coefficient (fraction of base frequency)
        b = rng.uniform(-0.05, 0.05) * f_0  # Linear coefficient

        def f_evol(time):
            return a * time**2 + b * time + f_0
    ...
    if perturbation_type == "sinusoidal":
        ...
        amp_mod = (
            rng.uniform(0.01, 0.01) * f_0
        )  # Modulation amplitude (fraction of base frequency)

        def f_pert(time):
            return amp_mod * np.sin(2 * np.pi * freq_mod * time)
    else:  # random

        def rand_pert(time):
            return rng.normal(
                0, 0.001 * f_0, len(time)
            )  # Random noise (fraction of base frequency)
        ...
        def f_pert(time):
            return rand_pert(time)

    # 4. Combine Evolution and Perturbation
    def f_t(time):
        return f_evol(time) + (
            f_pert(time) if doPerturbation else 0
        )  # For plotting the underlying evolution without perturbation
...
def gen_amplitude_evolution(freq_avg, doPerturbation=True):
    ...
    if evolution_type == "linear":
        slope = (
            rng.uniform(-0.05, 0.05) * I_0
        )  # Linear slope (fraction of base amplitude)

        def I_evol(time):
            return slope * time + I_0

    else:  # polynomial
        a = (
            rng.uniform(-0.001, 0.001) * I_0
        )  # Quadratic coefficient (fraction of base amplitude)
        b = rng.uniform(-0.01, 0.01) * I_0  # Linear coefficient

        def I_evol(time):
            return a * time**2 + b * time + I_0
    ...
    if perturbation_type == "sinusoidal":
        ...
        amp_mod = (
            rng.uniform(0.01, 0.1) * I_0
        )  # Modulation amplitude (fraction of base amplitude)

        def I_pert(time):
            return amp_mod * np.sin(2 * np.pi * freq_mod * time)
    else:  # random
        # Random noise (fraction of base amplitude)
        def rand_pert(time):
            return rng.normal(
                0, 0.01 * I_0, len(time)
            )  # Random noise (fraction of base amplitude)
        ...
        def I_pert(time):
            return rand_pert(time)  # Apply filter (zero-phase)

    # 4. Combine Evolution and Perturbation
    def I_t(time):
        return np.abs(I_evol(time) + (I_pert(time) if doPerturbation else 0))
...
def __get_plausible_mn_values(gEQDSK_file, max_n=15, max_m=15, max_modes=5):
    ...
    with open(working_directory + "input_data/" + gEQDSK_file, "r") as f:
        eqdsk = geqdsk.read(f)
    ...
    for n in range(1, max_n + 1) if np.size(max_n) == 1 else max_n:
        ...
        if upper_m > np.max(max_m):
            upper_m = np.max(max_m)
        if upper_m > n * 3:
            upper_m = n * 3
        if upper_m > n + 4:
            upper_m = n + 4
        ...
        for m in range(lower_m, upper_m + 1):
            ...
            if eqdsk.qpsi[0] < m / n < eqdsk.qpsi[-1]:
                plausible_mode_pairs.append((m, n))

    plausible_mode_pairs = np.array(plausible_mode_pairs)
    out_mode_pairs = plausible_mode_pairs[
        np.random.choice(
            len(plausible_mode_pairs),
            size=np.random.randint(1, max_modes + 1),
            replace=False,
        ).tolist()
    ]
    return (
        out_mode_pairs[:, 0].tolist(),
        out_mode_pairs[:, 1].tolist(),
        plausible_mode_pairs,
    )  #
...
def __build_geqdsks(
    n_equilibria,
    shot_list_file="../C-Mod/C_Mod_Shot_List_with_TAEs_Sheet1.csv",
    output_file=working_directory + "input_data/gEQDSK_files/",
    time_range=[0.75, 1.25],
    debug=True,
    justLoad=False,
):
    ...
    #####################
    shotnos = np.loadtxt(
        shot_list_file, skiprows=1, delimiter=",", usecols=0, dtype=int
    )
    ...
    times = np.random.choice(
        np.linspace(time_range[0], time_range[1], 100 + 1, endpoint=True),
        int(n_equilibria * 1.2),
        replace=True,
    )

    g_files = [
        "g%d.%04d" % (shots[ind], times[ind] * 1e3) for ind in range(n_equilibria)
    ]

    g_files_out = []  # separate out file list, to account for gEQDSKs that fail to generate
    for ind, g_file in enumerate(g_files):
        if debug:
            print(f"Processing shot {shots[ind]} at time {times[ind]}: {g_file}")
    ...
    if len(g_files_out) < n_equilibria:
        raise ValueError(
            f"Requested {n_equilibria} equilibria, but only {len(g_files_out)} successfully generated."
        )
