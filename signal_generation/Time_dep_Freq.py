...
if debug_plot:
    ...
    for i,t in enumerate(time):ax.plot(t*1e3,f_k[i]*1e-3,'*',c=plt.get_cmap('viridis')(norm(I_k[i])))
    for i,t in enumerate(time):ax.plot(t*1e3,f_ae[i]*1e-3,'*',c=plt.get_cmap('viridis')(norm(I_ae[i])))
    ax.grid()
