...
def I_KM(t):
    10 - 1 * np.sin(2 * np.pi * t * 1e3)


def F_KM(t):
    2 * np.pi * 7e3 * t + 2e3 * np.sin(2 * np.pi * t * 1e3) / 1e3


def F_KM_plot(t):
    7e3 + 2e3 * np.sin(2 * np.pi * t * 1e3)


# AE
def I_AE(t, dead_time=0.2):
    # Break into repeating bands
    local_t = t % period
    I_out = np.zeros((len(t)))
    I_out[local_t < (period) * (1 - dead_time)] = 5 + 1 * local_t[
        local_t < (period) * (1 - dead_time)
    ] / ((period) * (1 - dead_time))

    t_shift = local_t[local_t >= (period) * (1 - dead_time)]
    step_val = np.where(local_t >= (period) * (1 - dead_time))[0][0] - 1
    I_out[local_t >= (period) * (1 - dead_time)] = I_out[step_val] * np.exp(
        -1e5 * (t_shift - (period) * (1 - dead_time))
    )
    return I_out


def F_AE(t, dead_time=0.2, lam=5e3, f_carrier=50e3, f_mod=30e3):
    # Break into repeating bands
    t = np.array(t)
    local_t = t % (period)
    ...
    t_shift = local_t[
        local_t < (period) * (1 - dead_time)
    ]  # /((1/periods)*(1-dead_time))
    f_out[local_t < (period) * (1 - dead_time)] = (
        2
        * np.pi
        * (f_carrier * t_shift - f_mod * (t_shift + np.exp(-t_shift * lam) / lam))
    )
    # 300e3 - 30e3 * (1-np.exp(-t_shift*4))

    t_shift = local_t[local_t >= (period) * (1 - dead_time)]
    step_val = np.where(local_t >= (period) * (1 - dead_time))[0][0] - 1
    f_out[local_t >= (period) * (1 - dead_time)] = f_out[step_val] * np.exp(
        -1 * (t_shift - (period) * (1 - dead_time))
    )
    return f_out


def F_AE_plot(t, dead_time=0.2, lam=5e3, f_carrier=50e3, f_mod=30e3):
    ...
    f_out[local_t < (period) * (1 - dead_time)] = f_carrier - f_mod * (
        1 - np.exp(-t_shift * lam)
    )
    # 300e3 - 30e3 * (1-np.exp(-t_shift*4))
...
def gen_coupled_freq(
    time,
    periods,
    dead_fraction,
    f_mod,
    I_mod,
    I_chirp_smooth_percent=0.05,
    random_seed=None,
):
    ...
    chirp_bands = [
        np.arange(chirp_boundary[ind], chirp_boundary[ind + 1], dtype=int)
    ...
    for ind, band in enumerate(chirp_bands):
        local_t = np.linspace(
            0, 1, int(len(band) * (1 - dead_fraction)), endpoint=False
        )
        ...
        I_chirp[0:I_chirp_smooth] = np.linspace(
            0, I_chirp[I_chirp_smooth], I_chirp_smooth
        )
        I_chirp[-I_chirp_smooth:] = np.linspace(
            I_chirp[-I_chirp_smooth], 0, I_chirp_smooth
        )

        f_out[band[0 : len(local_t)]] = (
            f_chirp * 2 * np.pi * time[chirp_bands[0][0 : len(local_t)]]
        )
...
def debug_mode_frequency_plot(
    ...
    norm = colors.Normalize(
        vmin=np.nanmin(np.concatenate((I_out_1, I_out_2, I_out_3))),
    ...
    for i, t in enumerate(time):
        ax.plot(
            ...
            f_out_plot_1[i] * 1e-3,
            "*",
            c=plt.get_cmap("viridis")(norm(I_out_1[i])),
        )
        ax.plot(
            t * 1e3,
            f_out_plot_2[i] * 1e-3,
            "*",
            c=plt.get_cmap("viridis")(norm(I_out_2[i])),
        )
        ax.plot(
            t * 1e3,
            f_out_plot_3[i] * 1e-3,
            "*",
            c=plt.get_cmap("viridis")(norm(I_out_3[i])),
        )
    ...
    ax.set_ylabel("f [kHz]")
    fig.colorbar(
        cm.ScalarMappable(norm=norm, cmap="viridis"), ax=ax, label="Current [A]"
    )
    ...
    plt.show()
    plt.savefig(
        "../output_plots/multimode_target_spectrum%s.pdf" % save_ext, transparent=True
    )
...
if __name__ == "__main__":
    ...
    f_out, I_out, f_out_plot = gen_coupled_freq(
        time, periods, dead_fraction, f_mod, I_mod
    )
