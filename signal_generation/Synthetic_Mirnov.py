...
def gen_synthetic_Mirnov(
    ...
    sensors = gen_Sensors_Updated(
        select_sensor=sensor_set, cmod_shot=cmod_shot, skipBP=True, debug=debug
    )

    # Get finite element Mesh
    tw_mesh, sensor_obj, Mc, eig_vals, eig_vecs, L_inv = get_mesh(
        mesh_file, xml_filename, params, sensor_set, oft_env=oft_env, debug=debug
    )

    # Run time dependent simulation
    if not plotOnly:
        if scan_in_freq:
            _sensors_bode = run_frequency_scan(
                tw_mesh,
                params,
                sensor_set,
                mesh_file,
            ...
            coil_currs = run_td(
                ...
                archiveExt,
                doPlot=doPlot,
                doPlot_Currents=doPlot,
            )

    if not doPlot:
        return coil_currs, filament_coords, sensors
    ...
    scale = makePlots(
        ...
        file_geqdsk,
        sensor_set,
        plot_B_surf=not plotOnly,
        debug=debug,
        scan_in_freq=scan_in_freq,
        clim_J=clim_J,
...
def get_mesh(mesh_file, filament_file, params, sensor_set, oft_env=None, debug=True):
    ...
    tw_mesh.setup_model(
        mesh_file=f"{j_id}input_data/" + mesh_file,
        xml_filename=f"{j_id}input_data/" + filament_file,
    )
    ...
    Msensor, Msc, sensor_obj = tw_mesh.compute_Msensor(
        f"{j_id}input_data/floops_%s.loc" % sensor_set
    )
    ...
    tw_mesh.compute_Lmat(
        use_hodlr=True,
        cache_file=f"{j_id}input_data/HOLDR_L_{mesh_file}_{sensor_set}.save",
    )
...
################################################################################################
def gen_OFT_filement_and_eta_file(
    filament_file, params, filament_coords, eta="1.8E-5, 3.6E-5, 2.4E-5"
):
...
################################################################################################
def gen_coil_currs(
    param, debug=True, doSave=False, wind_in="theta", scan_in_freq=False
):
    ...
    if type(m) is int:
        m = [m]  # Convert type to correct format
    ...
    if type(pts) is int:
        num_filaments = pts * len(starting_angle)
    else:
        num_filaments = sum([len(theta) for theta in starting_angle])
    if debug:
        print("Number of filaments: %d" % num_filaments)
    coil_currs = np.zeros((time.size, num_filaments + 1))

    # Time vector in first column
    coil_currs[:, 0] = time
    if debug:
        print("Initialized coil currents array with shape (%d,%d)" % coil_currs.shape)
    ...
    if not scan_in_freq:
        ...
        cumulative_filament_coords = np.cumsum(
            [len(starting_angle_) for starting_angle_ in starting_angle]
        )
        cumulative_filament_coords = np.insert(
            cumulative_filament_coords, 0, 0
        )  # prepend 0 to cumulative coords
        for ind_filament, starting_angle_list in enumerate(starting_angle):
            if debug:
                print(
                    f"Assigning currents to filaments for m/n = {m[ind_filament]}/{n[ind_filament]}"
                )

            # Get (potentially time dependent) mode amplitude
            if type(param["I"]) is int:
                mode_amp = param["I"] * np.ones((nsteps + 1,))
            elif type(param["I"]) is list:
                mode_amp = param["I"][ind_filament]
            else:
                mode_amp = param["I"]

            # Potentially time dependent frequency
            if type(param["f"]) is float:
                mode_freq = param["f"] * 2 * np.pi * time  # always just return f
            # assume frequency is a function taken [0,1] as the argument
            elif type(param["f"]) is list:
                mode_freq = param["f"][ind_filament]
            else:
                mode_freq = param["f"]  #

            # set initial mode phase based on starting angle position for a given filament
            initial_mode_phase = (
                m[ind_filament] if wind_in == "phi" else n[ind_filament]
            )
            ...
            for ind, t in enumerate(time):
                ...
                coil_currs[
                    ind,
                    cumulative_filament_coords[ind_filament]
                    + 1 : cumulative_filament_coords[ind_filament + 1] + 1,
                ] = np.random.normal(1, noise_envelope, len(starting_angle_list)) * [
                    mode_amp[ind]
                    * np.cos(initial_mode_phase * initial_angle + mode_freq[ind])
                    for initial_angle in starting_angle_list
                ]

    # Save coil currents to .npy for manual inspection later
    if doSave:
        np.savez(
            "../data_output/coil_currs.npz",
            coil_currs=coil_currs,
            m=m,
            n=n,
            n_pts=n_pts,
            m_pts=m_pts,
...
def __do_save_output(
    ...
    f_save = (
        working_directory
        + "../data_output/%sfloops_filament_%s_m-n_%s_f_%s_%s%s"
    ...
    for key in hist_file.keys():
        data_out[key] = hist_file[key].tolist()
...
def run_frequency_scan(
    tw_mesh, params, sensor_set, mesh_file, sensor_obj, doSave_Bode, save_ext
):
...
def makePlots(
    ...
    if type(m) is int:
        m = [m]  # Convert type to correct format
    if type(n) is int:
        n = [n]  # Convert type to correct format
    ...
    if plot_B_surf:
        ...
        p.add_mesh(grid, color="white", opacity=0.1, show_edges=True)
    ...
    cumulative_filament_coords = np.cumsum(
        [len(filament) for filament in filament_coords]
    )
    cumulative_filament_coords = np.insert(
        cumulative_filament_coords, 0, 0
    )  # prepend 0 to cumulative coords
    while not np.any(coil_currs[t_pt, :]):
        t_pt += 1  # Check for first non-zero timepoint
        ...
        for ind, filament in enumerate(filament_list):
            ...
            p.add_mesh(
                ...
                color=plt.get_cmap("plasma")(
                    (
                        coil_currs[t_pt, 1 + ind + cumulative_filament_coords[ind_mn]]
                        / np.max(coil_currs[t_pt, :])
                        + 1
                    )
                    / 2
                ),
                line_width=10,
                render_points_as_spheres=True,
                label="Filament %d/%d" % (m[ind_mn], n[ind_mn]) if ind == 0 else None,
                opacity=1 - 0.5 * ind_mn / len(filament_coords),
            )
    ...
    if debug:
        print("Saved figure:" + doSave + "Mesh_and_Filaments%s.png" % save_Ext)
...
if __name__ == "__main__":
    ...
    clim_J = [0, 0.5]  # Color limits for eddy current plot
    doSave_Bode = True
