...
def gen_filament_coords(params, wind_in="phi"):
    ...
    starting_angle = []
    winding_angle = []
    if type(m_pts) is int:
        m_pts = [m_pts] * len(m)
    if len(m_pts) != len(m):
        m_pts = m_pts * len(m)
    if type(n_pts) is int:
        n_pts = [n_pts] * len(m_pts)
    if len(n_pts) != len(m):
        n_pts = n_pts * len(m)
    ...
    for ind_m, m_ in enumerate(m if type(m) is list else [m]):
        ...
        if wind_in == "phi":
            starting_angle.append(
                np.linspace(
                    0, 2 * np.pi / m_local * n_local, m_pts[ind_m], endpoint=True
                )
            )
            winding_angle.append(
                np.linspace(0, m_local * 2 * np.pi, n_pts[ind_m], endpoint=True)
            )
        elif wind_in == "theta":
            starting_angle.append(
                np.linspace(0, 2 * np.pi / n_local, n_pts[ind_m], endpoint=False)
            )
...
########################
def calc_filament_coords_geqdsk(
    file_geqdsk,
    theta,
    phi,
    params,
    ...
    coords = []
    for ind_m, m in enumerate(
        params["m"] if type(params["m"]) is list else [params["m"]]
    ):
        ...
        if file_geqdsk is None:  # Assigning circiular flux surface, fixed radius at a
            ...
            def r_theta(theta):
                return [a] * len(theta)  # running circular approximation
            ...
            contour = np.array(
                [np.argmin(np.abs(R_eq - a * np.cos(t) - rmagx)) for t in theta_r]
            )
            contour = np.vstack(
                (
                    contour,
                    [
                        np.argmin(np.abs(Z_eq - a * np.sin(t) - zmagx).squeeze())
                        for t in theta_r
                    ],
                )
            )
            contour = contour[
                ::-1
            ].T  # flip for consistency with gEqdsk contour detection

            r_norm = [a] * 100  # mode exists at a constant radius

        else:  # Using geqdsk equilibrium to locate flux surfaces
            # Load eqdsk
            with open(working_directory + "input_data/" + file_geqdsk, "r") as f:
                eqdsk = geqdsk.read(f)
            ...
            try:
                contour = np.squeeze(
                    contour
                )  # Check if only the main contour was found
            except Exception:
            ...
            def r_theta_(theta):
                return np.polyval(np.polyfit(theta_r, r_norm, 11), theta % (2 * np.pi))

            def r_theta(theta):
                return r_theta_(theta % (2 * np.pi))  # Radial coordinate vs theta
        ...
        if debug:
            if False:  # Old way
                plt.close(
                    ...
                    % (m, n, debug if type(debug) is str else "")
                )
                fig, ax = plt.subplots(
                    1,
                    3,
                    tight_layout=True,
                    figsize=(7, 4),
                    num="filament_debug_%d-%d%s"
                    % (m, n, debug if type(debug) is str else ""),
                )
                ax[0].plot(R_eq[contour[:, 1]], Z_eq[contour[:, 0]], "*")
                theta_fit = np.linspace(0, 2 * np.pi, 50)  # -np.pi
                ax[0].plot(
                    r_theta(theta_fit) * np.cos(theta_fit) + rmagx,
                    r_theta(theta_fit) * np.sin(theta_fit) + zmagx,
                    alpha=0.6,
                )

                ax[1].plot(
                    theta_r / (2 * np.pi), r_norm, "*", label=r"$\psi_{%d/%d}$" % (m, n)
                )
                ax[1].plot(
                    theta_fit / (2 * np.pi),
                    r_theta(theta_fit),
...
def wind_in_theta(file_geqdsk, m, n, debug=False):
    ...
    filament_points, filament_etas = eq_filament.trace(
        trace_type=EquilibriumFilamentTracer.TraceType.AVERAGE, num_points=300
    )
    ...
    if debug:
        ...
        plt.plot(
            [0, len(filament_points)], [0, filament_points[-1, 1] / (2 * np.pi)], "--k"
        )
        plt.grid()
...
if __name__ == "__main__":
    ...
    coords = calc_filament_coords_geqdsk(
        file_geqdsk, theta, phi, params, debug="_Fixed_Radius"
    )
