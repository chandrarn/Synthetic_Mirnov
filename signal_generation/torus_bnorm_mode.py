...
r_grid, bnorm, nfp = build_torus_bnorm_grid(
    "tCurr_mode.dat", ntheta, nphi, resample_type="theta", use_spline=False
)
...
if nfp > 1:
    ...
    for j in range(2):
        output[j, :nelems_new] = np.dot(
            Linv, np.r_[flux_flat[j, 1 : -bnorm.shape[2]], 0.0, 0.0]
        )
