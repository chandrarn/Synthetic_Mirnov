...
import xarray as xr
from M3DC1_to_Bnorm import calculate_normal_vector, save_Bnorm
import matplotlib.pyplot as plt
from freeqdsk import geqdsk
from sys import path; path.append('/home/rianc/Documents/eqtools-1.0/')
...
def plot_B(br_file='br_0000',bth_file='bth_0000',m=[9,10,11,12],eqdsk_file='g1051202011.1000'):
    ...
    #psi_normalized = get_psi_grid('input_data/'+eqdsk_file)

    #psi_norm = get_psi_grid('input_data/'+eqdsk_file)
    ...
    fig.savefig('../output_plots/FAR3D_Eigenfunction_Original.pdf',transparent=True)
    plt.show()
...
def map_psi_to_equilibrium(br_file='br_0000',bth_file='bth_0000',\
    ...
    psi_normalized,R,Z, rmagx,zmagx = get_psi_grid('input_data/'+eqdsk_file)

    # Get plasma boundary contour
    inside_inds,_ = gen_psi_boundary_contour(psi_normalized, R, Z,rmagx,zmagx,psi_LCFS=0.95)
    if debug: plt.figure();plt.plot(R[inside_inds[:,0]]+rmagx,Z[inside_inds[:,1]]+zmagx,'k*',label='Plasma Domain')
    ...
    _,_, B_r_RZ_downsamp_sin = interpolate_B(theta_inside,R_inside,Z_inside,dat_r[:,1:],indices,m,np.pi/2)
    _,_, B_th_RZ_downsamp_cos = interpolate_B(theta_inside,R_inside,Z_inside,dat_th[:,0:],indices,m)
    _,_, B_th_RZ_downsamp_sin = interpolate_B(theta_inside,R_inside,Z_inside,dat_th[:,1:],indices,m,np.pi/2)
    ...
    if debug:
        ...
        fig.savefig('../output_plots/FAR3D_Eigenfunction_Equilibrium.pdf',transparent=True)
        plt.show()
