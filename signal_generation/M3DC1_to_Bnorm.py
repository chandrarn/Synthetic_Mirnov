...
path.append("/orcd/home/002/rianc/")
try:
    import C1py
    import fio_py
except Exception:
...
def convert_to_Bnorm(C1file_name, n, npts):
    ...
    if (
        gethostname()[:4] == "orcd" or gethostname()[:4] == "node"
    ):  # if running on Engaging
        psi, B1, B2, R, Z, rmagx, zmagx, PsiLCFS = get_fields_from_C1(
            C1file_name, n, False
        )
    else:
    ...
    B1_norm, R_contour, Z_contour = calculate_normal_vector(
        psi, R, Z, B1, rmagx, zmagx, PsiLCFS
    )[:3]
    ...
    save_Bnorm(
        "C1"
        if (gethostname()[:4] == "orcd" or gethostname()[:4] == "node")
        else "input_data/",
        R_contour,
        Z_contour,
        B1_norm,
...
def get_fields_from_C1(filename, n, saveNetCDF=True):
    ...
    # R = b_field.coords["R"].values
    # Z = b_field.coords["Z"]

    # C1 file data
    isrc = fio_py.open_source(fio_py.FIO_M3DC1_SOURCE, filename)
    psi_lcfs = fio_py.eval_series(fio_py.get_series(isrc, fio_py.FIO_LCFS_PSI), 0.0)
...
def return_local(n):
    ...
    R = b_field_1.coords["R"].values
    Z = b_field_1.coords["Z"].values
    psi = psi.data
...
def __debug_local_bnorm(doSave=""):
    ...
    b_norm, R_sig, Z_sig, all_norm, contour, all_dr, B = calculate_normal_vector(
        psi, R, Z, B1, rmagx, zmagx, PsiLCFS, doPlot=True, doSave=doSave
    )
...
def calculate_normal_vector(
    ...
    try:
        ...
        print("Multiple contours found, selecting closest to magnetic axis")
...
def __b_norm_debug_plots(
    ...
    for j in range(3):
        ...
        for i in range(len(contour)):
            ...
            ax[j].plot(
                [R_sig[i], R_sig[i] + vec[0] * 0.05],
                [Z_sig[i], Z_sig[i] + vec[2] * 0.05],
                "k-",
                zorder=5,
                alpha=0.1,
            )
    ...
    fig.colorbar(
        ...
        label=r"$||\tilde{\mathrm{B}}_{\hat{n}}||$ [G]",
        ax=ax[2],
    )
    fig.colorbar(
        cm.ScalarMappable(norm=norm_2, cmap=cm.get_cmap("viridis")),
        label=r"$||\tilde{\mathrm{B}}_j||$ [G]",
        ax=ax[1],
    )
    if doSave:
        fig.savefig(doSave + "B_Contour_Comparison.pdf", transparent=True)
    ############################################
    ...
    fig, ax = plt.subplots(
        ...
        sharex=True,
        sharey=True,
        num="Extraction_Verify",
        figsize=(8, 3),
    )
    all_norm = np.array(all_norm)
    v1 = np.abs(min(b_norm)) * 1.5
    v2 = np.abs(max(b_norm)) * 1.5
    ...
    for i in range(3):
        ...
        ax_tmp[-1].plot(
            xrange, all_norm[:, i], c=cm.get_cmap("tab10")(2), label=r"$\hat{n}_j$"
        )
        ax_tmp[-1].set_ylim([-1, 1])
        if i != 2:
            ax_tmp[-1].set_yticklabels([])
    ...
    ax[0].set_ylim(ylim)
    if doSave:
        fig.savefig(doSave + "B_Vector_Comparison.pdf", transparent=True)
...
def save_Bnorm(C1file_name, contour_R, contour_Z, B1_norm, B2_norm, n, npts):
    with open(
        (C1file_name + "_tCurr_mode.dat")
        if C1file_name
        else "input_data/tCurr_mode.dat",
        "w+",
    ) as f:
        f.write("{0} {1}\n".format(npts, n))
        for ind, r in enumerate(contour_R):
            f.write(
                "{0} {1} {2} {3}\n".format(
                    r, contour_Z[ind], B1_norm[ind], B2_norm[ind]
                )
            )
    print(
        "Saving B-Norm: %s"
...
def __plot_time_series(t_points=5):
    ...
    for i in np.arange(1, 5):
        ...
        calculate_normal_vector(
            ...
            samp_pts=200,
            doPlot=True,
            ax=ax[np.unravel_index(i - 1, (2, 2))],
            fig=fig,
        )
...
def __plot_time_series_im(t_points=5, B_index=0):
    ...
    fig, ax = plt.subplots(
        2, 2, tight_layout=True, sharex=True, sharey=True, num="B_%s" % label[B_index]
    )
    psi = xr.open_dataset("psi.nc")
    psi_ = psi.to_array().data.squeeze()
    R = psi.coords["R"].values
    ...
    for i in np.arange(1, 5):
        b_field = xr.open_dataset("B_Field_%d.nc" % i)
        B = b_field.to_array().data.squeeze()
        ax[np.unravel_index(i - 1, (2, 2))].imshow(
            B[B_index].T, origin="lower", extent=extent
        )
        ax[np.unravel_index(i - 1, (2, 2))].contour(
            psi_.T, [1], hold="on", origin="lower", extent=extent, alpha=0.7
        )
        norm = Normalize(vmin=np.min(B[B_index]), vmax=np.max(B[B_index]))
        fig.colorbar(
            cm.ScalarMappable(norm=norm, cmap=cm.get_cmap("viridis")),
            label="$\mathrm{B_%s}$ [?]" % label[B_index],
            ax=ax[np.unravel_index(i - 1, (2, 2))],
        )
    for i in range(2):
...
def __finDiff(signal, ind, order, deriv, periodic=False):  # Finite difference stencil
    ...
    if periodic:
        ...
        if len(signal) - ind < (order + 1):
            s_return = np.append(
                s_return, np.arange(0, order + 1 - (len(signal) - ind))
            )

    else:
        s = np.arange(
            -order if ind >= order else -ind,
            (order + 1) if len(signal) - ind >= (order + 1) else len(signal) - ind,
        )
        s_return = s + ind
    # Build with automatic s generator: input: order, derivative
    if not len(s) > deriv:
        raise SyntaxError("Insufficient Points for Derivative")
